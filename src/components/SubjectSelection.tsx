...
import { ArrowLeft, BookOpen, Globe, Newspaper, Trophy, Brain, Atom, Church, Palette } from "lucide-react";
          ...
          {subjects.map((subject, index) => {
            ...
            return (
              <motion.button
                ...
                className="group glass rounded-2xl p-6 md:p-8 flex flex-col items-center justify-center gap-4 cursor-pointer border-2 border-border hover:border-primary transition-all duration-300 hover:shadow-lg hover:shadow-primary/20"
                ...
                whileTap={{ scale: 0.97 }}
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-xl bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                  <Icon className="w-7 h-7 md:w-8 md:h-8 text-primary" />
                </div>
                <span className="font-display text-base md:text-lg font-bold text-foreground text-center leading-tight">
                  {subject.name}
                </span>
              </motion.button>
            );
